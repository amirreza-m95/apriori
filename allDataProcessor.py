...
def apriori(dataframe, supTreshhold, confTreshhold):
    ...
    oneItems.show()
    twoItems = twoItemset(dataframe, oneItems, 0.009)
    ...
    while not twoItems:
        ...
        print('two items collect:' + str(twoItems.collect()))
        twoItems.show()
    threeItemset = otherItemsets(twoItems , 0.008)
    print('two items collect:' + str(threeItemset.collect()))
    threeItemset.show()
    while threeItemset:
...
if __name__ == "__main__":
    spark = SparkSession \
        ...
        .master("local[*]")\
        .getOrCreate()
    dataframe = _input("testdataset.csv", spark)
    m = numOfDistinctItems(dataframe, "InvoiceNo")
    apriori(dataframe, 0.03, 0.05)
